...
def cluster_topface(all_hots,cutoff=0.8):
    hots = [pro for pro,hots in all_hots.iteritems()]
    seeds = [pro for pro,hots in all_hots.iteritems() if len(hots) == 7] + \
            [pro for pro,hots in all_hots.iteritems() if len(hots) == 8] + \
            [pro for pro,hots in all_hots.iteritems() if len(hots) == 6]
    clusters = []
    n = 0
    for p in seeds:
        clusters.append([p])
        tem = all_hots[p]
        for pro,hots in hots:
            if align_hot(tem,all_hots[pro]) >= cutoff:
                clusters[n].append(pro)
        n += 1
        hots = list(set(hots).difference(set(cluster[n])))
