...
class PatchSearch:
    ...
    def check_patches(self, shape_k, shape_v, patches, surround_patches):
        ...
        def validify(patch):
            ...
            for i, p in enumerate(patch):
                if shape_v[i][1] == 0 and p in ['T', 'S', 'N']:
                    return 0
    ...
    def get_best(self):
        ...
        self.best_patch = [sta[0][1],sta[0][2],sta[0][3],sta[0][0]]

        return [sta[0][1],sta[0][2],sta[0][3],sta[0][0]]
...
def get_out_search():
    ...
    with open(sys.argv[-1]) as wdsp_f:
        ...
        while pro_num > CUTOFF:
            a = PatchSearch(pros,seqs,wdsps,hotspots,CUTOFF)
            a.get_patches()
            a.classify_patches()
