...
def main():
    ...
    propellers = pickle.load(open('pfam_propellers.pickle','r'))
    ...
    h = sns.barplot(y='Propellers',x='Num',data=wd,color='b')
    h.figure.subplots_adjust(top=0.9,bottom=0.05,left=0.20,right=0.95)
    ax.set(ylabel='Propellers',xlabel='Num',title='Num of Different Propellers in Uniprot according to Pfam Annotation')
    ...
    f,ax = plt.subplots()
    sns.set_context(rc={'patch.linewidth':0.0})
    wd40_names = ['WD40','WD40_3','WD40_4','ANAPC4_WD40','Ge1_WD40','PALB2_WD40','WD40_like']
    ...
    wd = pd.DataFrame({'WD40_entries':wd40_names,'Num':map(len,[wd40s_total for i in range(len(wd40_names))])})
    sns.barplot(x='WD40_entries',y='Num',data=wd,color='lightblue')
    ...
    for i in range(0,len(wd40s_large)):
        for j in range(0,len(wd40s_large)):
            if j > i:
                set1 = set(wd40s_large[i])
                set2 = set(wd40s_large[j])
                set10 = len(set1.difference(set2))
                set12 = len(set1.intersection(set2))
                set02 = len(set2.difference(set1))
                v = venn2(subsets={'10':1,'11':1,'01':1},set_labels=(wd40_names[i],wd40_names[j]))
                v.get_label_by_id('10').set_text(str(set10))
                v.get_label_by_id('11').set_text(str(set12))
                v.get_label_by_id('01').set_text(str(set02))
                plt.savefig(wd40_names[i]+'_'+wd40_names[j]+'.png',dpi=300)
                plt.close('all')
    ...
    h.figure.subplots_adjust(top=0.88,bottom=0.10,left=0.20,right=0.98)
