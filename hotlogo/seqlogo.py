...
COLORS = {'A': 'black', 'C': 'green', 'D': 'red', 'E': 'red', 'F': 'black', 'G': 'green', 'H': 'blue', 'I': 'black', 'K': 'blue', 'L': 'black',
          'M': 'black', 'N': 'green', 'P': 'black', 'Q': 'green', 'R': 'blue', 'S': 'green', 'T': 'green', 'V': 'black', 'W': 'black', 'Y': 'green'}
# chemical colors
...
def read_glyph():
    ...
    for i in range(65, 91):
        ...
        glyphs[chr(i)] = [path.getAttribute('d')
                          for path in svg.getElementsByTagName('path')][-1]

    # with open('test.txt','w') as w
        # for k,v in glyphs.iteritems():
            # print >> w_f,'GLYPHS["{0}"]="{1}"'.format(k,v)
    return glyphs
