...
"""
workflow
given a list of clusters of pros
1. calculate seq similarity and topface similarity of pros in the same cluster
2. plot logo of hotspots
3. plot topface_seq scatter plot

usage: python topface_seq.py pro_clusters.list all.wdsp
"""
...
def main():
    ...
    with open(sys.argv(-2)) as clusters_f:
        "
        format as follows:
        100 shape_3_2_1_a HRG 4 R9F4I5,Q0BDJ6,...
        "
        lines = clusters_f.readlines()
        lines = [line.rstrip('\r\n') for line in lines]
        lines = [line for line in lines if line]
        lines = [(line[0],line[1],line[2],line[3],line[4].split(',')) for line in lines]
    with open(sys.argv[-1]) as wdsp_f:
        wdsp = Wdsp(wdsp_f)
        all_hots = wdsp.hotspots
        all_seqs = wdsp.seqs


    regressions = []
    for num,shape,patch,_,pros in lines:
        filename = str(num)+'_'+shape+'_'+patch
        hots = [[pro,all_hots[pro]] for pro in pros]
        seqs = [[pro,all_seqs[pro]] for pro in pros]
        ...
        plot_scatter(seqs_score_pair,hots_score_pair,filename+'_scatter')
